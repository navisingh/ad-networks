...
- (void)getAd {
    int adUnit = INMOBI_AD_UNIT_320x48;
	if (UI_USER_INTERFACE_IDIOM() == UIUserInterfaceIdiomPad)
        adUnit = INMOBI_AD_UNIT_728x90;
    InMobiAdView *inMobiView = [InMobiAdView requestAdUnit:adUnit withDelegate:self];
    self.adNetworkView = inMobiView;
}

- (void)stopBeingDelegate {
    // no way to set inMobiView's delegate to nil
    InMobiAdView *inmobiAdView = (InMobiAdView *)self.adNetworkView;
    if (inmobiAdView != nil) {
